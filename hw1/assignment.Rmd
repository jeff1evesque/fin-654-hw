...
Determining a mechanism for measuring the test set, could involve defining a tolerance level (p=0.05). Specifically, if the predicted values were within the accepted tolerance, the prediction would be considered correct. Since not test set was utilized, forecasted `return` values can only be accepted at face value. Additionally, since predictions were performed on an adjusted dataset, an inverse log could rescale `return` value(s) to the original units.
...
Periodicity of particular events, resulting in increase of oil prices, can be mitigated by some of the above options. Specifically, when oil supply runs, perhaps due to war, or socio-political event(s), having stop measures can reduce temporal volatility.
